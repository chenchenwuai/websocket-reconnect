...
var WebsocketReconnect = function () {
  function WebsocketReconnect(url, protocols, options) {
    ...
    this._handleOpen = function (event) {
      ...
      var minUptime = _this._options.minUptime;
    ...
    this._handleMessage = function (event) {
      ...
      if (event.data === _this._options.pingMsg || event.data === '') {
        clearTimeout(_this._pongTimeoutId);

        _this._debug('pong');

        if (!_this._options.outputPingMsg) {
          return;
        }
      }
    ...
    this._handleReconnect = function () {
      _this._debug('reconnect event');

      var maxRetries = _this._options.maxRetries;

      if (_this.onreconnect) {
        _this.onreconnect({
          maxRetries: maxRetries,
          reconnectDelay: _this._nextConnectDelay,
          retryCount: _this._retryCount
        });
      }

      _this._listeners.reconnect.forEach(function (listener) {
        return _this._callEventListener({
          maxRetries: maxRetries,
          reconnectDelay: _this._nextConnectDelay,
          retryCount: _this._retryCount
        }, listener);
  ...
  Object.defineProperty(WebsocketReconnect.prototype, "messageQueue", {
    get: function get() {
  ...
  WebsocketReconnect.prototype.send = function (data) {
    if (this._ws && this._ws.readyState === this.OPEN) {
      ...
      var maxEnqueuedMessages = this._options.maxEnqueuedMessages;
  ...
  WebsocketReconnect.prototype._connect = function () {
    ...
    var _a = this._options,
        ...
        connectionTimeout = _a.connectionTimeout,
        WebSocket = _a.WebSocket;
    ...
    this._wait(delay).then(function () {
  ...
  WebsocketReconnect.prototype._wait = function (delay) {
    if (delay === void 0) {
      delay = 0;
    }
  ...
  WebsocketReconnect.prototype._getNextDelay = function () {
    var _a = this._options,
        reconnectionDelayGrowFactor = _a.reconnectionDelayGrowFactor,
        minReconnectionDelay = _a.minReconnectionDelay,
        maxReconnectionDelay = _a.maxReconnectionDelay;
    var delay = 0;
  ...
  WebsocketReconnect.prototype._heartStart = function () {
    ...
    var _a = this._options,
        enableHeartbeat = _a.enableHeartbeat,
        pingTimeout = _a.pingTimeout,
        pongTimeout = _a.pongTimeout,
        pingMsg = _a.pingMsg;
...
//# sourceMappingURL=websocket-reconnect.esm.js.map
